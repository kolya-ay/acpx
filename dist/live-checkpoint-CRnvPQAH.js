...
*/
function forwardMeta(payload) {
	return isRecord$1(payload._meta) ? { _meta: payload._meta } : {};
}
...
function extractAcpErrorInternal(value, depth) {
	if (depth > 5) return;
	if (value instanceof RequestError) return {
		code: value.code,
...
function isAcpMessageObject(value) {
	return asRecord$1(value) !== void 0;
}
...
function extractSessionUpdateNotification(message) {
	...
	const params = asRecord$1(message.params);
	if (!params) return;
...
function parsePromptStopReason(message) {
	...
	const record = asRecord$1(message.result);
	if (!record) return;
...
function parseJsonRpcErrorMessage(message) {
	...
	const errorRecord = asRecord$1(message.error);
...
//#endregion
//#region src/runtime/public/sdk-validators.ts
/**
...
*/
function validateAvailableCommand(raw) {
	if (!isRecord$1(raw)) return;
	...
	return {
		name,
		description,
		...isRecord$1(raw.input) ? { input: raw.input } : {},
		...forwardMeta(raw)
	};
}
//#endregion
...
function resolveStructuredPromptPayload(parsed) {
	if (asTrimmedString(parsed.method) === "session/update") {
		...
		if (isRecord$1(params) && isRecord$1(params.update)) {
...
function readToolContentText(value) {
	const record = isRecord$1(value) ? value : void 0;
	if (!record) return;
...
const TOOL_CONTENT_TEXT_READERS = {
	...
	resource: (record) => {
		const resource = isRecord$1(record.resource) ? record.resource : void 0;
...
function summarizeToolOutput(rawOutput) {
	...
	const record = isRecord$1(rawOutput) ? rawOutput : void 0;
	if (!record) return;
...
function usageUpdateEvent(payload) {
	...
	const meta = isRecord$1(payload._meta) ? payload._meta : void 0;
...
function currentModeUpdateEvent(payload) {
	const currentModeId = asTrimmedString(payload.currentModeId);
...
* `SessionConfigOption`.
*/
function isSessionConfigOption(entry) {
	return entry.type === "select" || entry.type === "boolean";
}
function configOptionUpdateEvent(payload) {
	return {
		type: "config_option_update",
		configOptions: (Array.isArray(payload.configOptions) ? payload.configOptions : []).filter((entry) => isRecord$1(entry) && isSessionConfigOption(entry)),
		...forwardMeta(payload)
	};
}
function planUpdateEvent(payload) {
	return {
		type: "plan",
		entries: (Array.isArray(payload.entries) ? payload.entries : []).map((entry) => validatePlanEntry(entry)).filter((entry) => entry !== void 0),
		...forwardMeta(payload)
	};
}
function readNullableString(payload, key) {
	if (!Object.hasOwn(payload, key)) return { present: false };
...
function sessionInfoUpdateEvent(payload) {
...
function availableCommandsUpdateEvent(payload) {
	return {
		...
		availableCommands: (Array.isArray(payload.availableCommands) ? payload.availableCommands : []).map((entry) => validateAvailableCommand(entry)).filter((entry) => entry !== void 0),
		...forwardMeta(payload)
	};
}
...
function parseAvailableCommands(raw) {
	if (!Array.isArray(raw)) return;
	const commands = raw.map((entry) => validateAvailableCommand(entry)).filter((entry) => entry !== void 0);
	return commands.length > 0 ? commands : void 0;
}
function parsePlanEntries(raw) {
	if (!Array.isArray(raw)) return;
	return raw.map((entry) => validatePlanEntry(entry)).filter((entry) => entry !== void 0);
}
function parseCurrentPlan(raw) {
...
function parseAcpxState(raw) {
	...
	if (currentPlan) state.current_plan = currentPlan;
...
function assignDesiredConfigOptions(state, raw) {
	const desiredConfigOptions = asRecord$1(raw);
...
function assignParsedSessionOptions(state, raw) {
	const sessionOptions = asRecord$1(raw);
...
function assignSessionOptionSystemPrompt(options, value) {
	...
	const appendRecord = asRecord$1(value);
...
async function readSessionIndex(sessionDir) {
	...
	try {
		...
		const record = asRecord$1(JSON.parse(payload));
...
function sourceToTokenUsage(source) {
	const usageRecord = asRecord$1(source);
...
function usageToTokenUsage(update) {
	const updateRecord = asRecord$1(update);
	const usageMeta = asRecord$1(updateRecord?._meta)?.usage;
	const source = asRecord$1(usageMeta) ?? updateRecord;
	if (!source) return;
...
function usageCost(update) {
	const cost = asRecord$1(asRecord$1(update)?.cost);
	if (!cost) return;
...
function cloneSessionAcpxState(state) {
	...
	return {
		...
		current_plan: state.current_plan ? deepClone(state.current_plan) : void 0,
...
const SESSION_UPDATE_HANDLERS = {
	...
	available_commands_update: (_conversation, acpx, update) => {
		if (update.sessionUpdate === "available_commands_update") acpx.available_commands = update.availableCommands;
	},
	...
	plan: (_conversation, acpx, update) => {
		if (update.sessionUpdate === "plan") acpx.current_plan = { entries: deepClone(update.entries) };
	}
};
...
async function resolveSessionMaxSegments(sessionId) {
	try {
		const configured = (await resolveSessionRecord(sessionId)).eventLog.max_segments;
...
async function removeStaleEventLock(lockPath) {
	try {
		...
		if (isProcessAlive$1(parsed.pid) && lockAgeMs <= EVENT_LOCK_STALE_MS) return false;
...
async function listSessionEvents(sessionId) {
	...
	for (let segment = maxSegments; segment >= 1; segment -= 1) {
		const filePath = sessionEventSegmentPath(sessionId, segment);
		if (await pathExists(filePath)) files.push(filePath);
	}
	const active = sessionEventActivePath(sessionId);
	if (await pathExists(active)) files.push(active);
	const events = [];
	for (const filePath of files) {
		const lines = (await fs$1.readFile(filePath, "utf8")).split("\n").filter((line) => line.trim().length > 0);
		for (const line of lines) try {
			const parsed = JSON.parse(line);
			if (isAcpJsonRpcMessage(parsed)) events.push(parsed);
		} catch {}
	}
	return events;
}
...
//#endregion
export { assertPersistedKeyPolicy as $, asTrimmedString as $t, reconcileAgentSessionId as A, withTimeout as At, permissionModeSatisfies as B, isAcpResourceNotFoundError as Bt, trimConversationForRuntime as C, mergePromptSourceWithText as Ct, sessionOptionsFromRecord as D, InterruptedError as Dt, persistSessionOptions as E, textPrompt as Et, assertRequestedModelSupported as F, exitCodeForOutputErrorCode as Ft, findSessionByDirectoryWalk as G, OUTPUT_ERROR_CODES as Gt, absolutePath as H, AUTH_POLICIES as Ht, isRequestedModelUnsupportedError as I, formatErrorMessage as It, listSessionsForAgent as J, PERMISSION_MODES as Jt, isoNow$2 as K, OUTPUT_ERROR_ORIGINS as Kt, modelStateFromConfigOptions as L, isRetryablePromptError as Lt, REQUESTED_MODEL_UNSUPPORTED_ERROR_CODE as M, listBuiltInAgents as Mt, REQUESTED_MODEL_UNSUPPORTED_REASONS as N, normalizeAgentName$1 as Nt, applyConversation as O, TimeoutError as Ot, RequestedModelUnsupportedError as P, resolveAgentCommand as Pt, writeSessionRecord as Q, asRecord$1 as Qt, splitCommandLine as R, normalizeOutputError as Rt, recordSessionUpdate as S, isPromptInput as St, mergeSessionOptions as T, promptToDisplayText as Tt, findGitRepositoryRoot as U, EXIT_CODES as Ut, DEFAULT_HISTORY_LIMIT as V, toAcpErrorPayload as Vt, findSession as W, NON_INTERACTIVE_PERMISSION_POLICIES as Wt, pruneSessions as X, SESSION_RECORD_SCHEMA as Xt, normalizeName as Y, PERMISSION_POLICY_ACTIONS as Yt, resolveSessionRecord as Z, asOptionalString as Zt, cloneSessionAcpxState as _, extractSessionUpdateNotification as _t, withConnectedSession as a, SessionNotFoundError as an, resetPerfMetrics as at, recordClientOperation as b, parsePromptStopReason as bt, currentModelIdFromSetModelResponse as c, parseSessionRecord as ct, setDesiredConfigOption as d, normalizeRuntimeSessionId as dt, deriveAgentFromSessionKey as en, formatPerfMetric as et, setDesiredModeId as f, defaultSessionEventLog as ft, applyConfigOptionsToState as g, isProcessAlive$1 as gt, applyConfigOptionsToRecord as h, sessionEventSegmentPath as ht, runPromptTurn as i, QueueProtocolError as in, recordPerfDuration as it, AcpClient as j, DEFAULT_AGENT_NAME as jt, applyLifecycleSnapshotToRecord as k, withInterrupt as kt, clearDesiredConfigOption as l, parsePromptEventLine as lt, syncAdvertisedModelState as m, sessionEventLockPath as mt, SessionEventWriter as n, AgentSpawnError as nn, incrementPerfCounter as nt, connectAndLoadSession as o, setPerfGauge as ot, setDesiredModelId as p, sessionEventActivePath as pt, listSessions as q, OUTPUT_FORMATS as qt, listSessionEvents as r, QueueConnectionError as rn, measurePerf as rt, applyRequestedModelIfAdvertised as s, startPerfTimer as st, LiveSessionCheckpoint as t, AcpxOperationalError as tn, getPerfMetricsSnapshot as tt, setCurrentModelId as u, serializeSessionRecordForDisk as ut, cloneSessionConversation as v, isAcpJsonRpcMessage as vt, advertisedModelState as w, parsePromptSource as wt, recordPromptSubmission as x, PromptInputValidationError as xt, createSessionConversation as y, parseJsonRpcErrorMessage as yt, getAcpxVersion as z, extractAcpError as zt };

//# sourceMappingURL=live-checkpoint-CRnvPQAH.js.map