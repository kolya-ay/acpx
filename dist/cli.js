#!/usr/bin/env node
import { a as runSessionQueueOwner, c as buildQueueOwnerArgOverride, h as __exportAll, l as flushPerfMetricsCapture, n as getTextErrorRemediationHints, o as runOnce, p as probeQueueOwnerHealth, t as createOutputFormatter, u as installPerfMetricsCapture } from "./output-xyHDwXyM.js";
import { Ct as mergePromptSourceWithText, Dt as InterruptedError, Et as textPrompt, Ft as exitCodeForOutputErrorCode, G as findSessionByDirectoryWalk, Mt as listBuiltInAgents, Nt as normalizeAgentName$1, Ot as TimeoutError, Q as writeSessionRecord, Qt as asRecord$1, Rt as normalizeOutputError, U as findGitRepositoryRoot, Ut as EXIT_CODES, W as findSession, Y as normalizeName, ct as parseSessionRecord, dt as normalizeRuntimeSessionId, ft as defaultSessionEventLog, gt as isProcessAlive, ht as sessionEventSegmentPath, jt as DEFAULT_AGENT_NAME, mt as sessionEventLockPath, nn as AgentSpawnError, pt as sessionEventActivePath, q as listSessions, qt as OUTPUT_FORMATS, tn as AcpxOperationalError, ut as serializeSessionRecordForDisk, vt as isAcpJsonRpcMessage, wt as parsePromptSource, xt as PromptInputValidationError, z as getAcpxVersion } from "./live-checkpoint-CRnvPQAH.js";
import { _ as resolveGlobalFlags, b as resolveSessionNameFromFlags, c as parseHistoryLimit, d as parseOutputFormat$1, f as parsePruneBeforeDate, g as resolveAgentInvocation, h as parseTtlSeconds, i as addSessionOption, l as parseMaxTurns, m as parseTimeoutSeconds, n as addPromptInputOption, o as parseAllowedTools, p as parseSessionName, r as addSessionNameOption, s as parseDaysOlderThan, t as addGlobalFlags, u as parseNonEmptyValue, v as resolveOutputPolicy, x as loadPermissionPolicySpec, y as resolvePermissionMode } from "./flags-CbgtgiTQ.js";
...
function loadSessionModule() {
	sessionModulePromise ??= import("./output-xyHDwXyM.js").then((n) => n.i);
	return sessionModulePromise;
}
function loadOutputModule() {
	outputModulePromise ??= import("./output-xyHDwXyM.js").then((n) => n.r);
...
function captureUsage(update, capture) {
	const source = asRecord(asRecord(update._meta)?.usage) ?? update;
	capture.usage = {
...
function registerFlowCommand(program, config) {
	program.command("flow").description("Run multi-step ACP workflows from flow files").command("run").description("Run a flow file").argument("<file>", "Flow module path").option("--input-json <json>", "Flow input as JSON").option("--input-file <path>", "Read flow input JSON from file").option("--default-agent <name>", "Default agent profile for ACP nodes without profile", (value) => parseNonEmptyValue("Default agent", value)).action(async function(file, flags) {
		const { handleFlowRun } = await import("./cli-BCT94dmM.js");
...
function assignQueueOwnerSessionOptions(options, rawSessionOptions) {
	const sessionOpts = asRecord$1(rawSessionOptions);
...
function assignSessionSystemPrompt(options, value) {
	...
	const systemPrompt = asRecord$1(value);
